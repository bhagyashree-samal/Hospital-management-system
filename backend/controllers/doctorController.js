...
//this function will remove duplicate slot and return the slot filter by using the time
...
export async function toggleAvailability(req,res) {
    ...
    if (!req.doctor || String(req.doctor._id || req.doctor.id) !== String(id)) {
      return res.status(403).json({ success: false, message: "Not authorized to update this doctor's availablility" });
    }
