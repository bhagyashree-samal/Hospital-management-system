...
const AppointmentPage = () => {
  ...
  const loadDoctorAppointments = useCallback(async () => {
    ...
    try {
      const resp = await API.get("/appointments/me", { headers });
      console.log("Response from /appointments/me:", resp?.data);
      ...
      if (user?.id) {
        try {
          ...
          const debugResp = await API.get(
            `/appointments/me?createdBy=${user.id}`,
            { headers },
          );
  ...
  const loadServiceAppointments = useCallback(async () => {
    ...
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    console.log("Outgoing headers for /service-appointment/me:", headers);

    try {
      const resp = await API.get("/service-appointment/me", { headers });
      console.log("Response from /service-appointment/me:", resp?.data);
      ...
      console.error(
        "Error calling /service-appointment/me:",
        err?.response?.data || err.message || err,
      );
      ...
      if (user?.id) {
        try {
          console.log("Attempting debug request with ?createdBy=", user.id);
          const debugResp = await API.get(
            `/service-appointment/me?createdBy=${user.id}`,
            { headers },
          );
                ...
                {appointmentData.map((item)=>(
                    <div key={item.id} className={cardStyles.doctorCard}>
                        ...
                        <div className={cardStyles.specialization}>{item.specialization}{" "}
                            {item.experience ? `• ${item.experience}`:" "}</div>
                            ...
                            {item.status === 'Rescheduled' && item.rescheduledTo ?(
                                <div className={cardStyles.rescheduledText}>Reschedule to {" "}
          ...
          {serviceData.map((srv) => (
            <div key={srv.id} className={cardStyles.serviceCard}>
              <div className={cardStyles.serviceImageContainer}>
                <img
                  src={srv.image || "/placeholder-doctor.png"}
                  alt={srv.name}
